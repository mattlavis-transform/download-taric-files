            ...
            if update_type in ("1", "3"):  # UPDATE or INSERT
                ...
                if validity_end_date is not None:
                    if validity_end_date < validity_start_date:
                        g.data_file.record_business_rule_violation("LA3", "The start date must be less than or equal to the end date.", operation, transaction_id, message_id, record_code, sub_record_code, language_id)

            if update_type == "3":  # INSERT
                # Business rule LA1
                if language_id in languages:
                    g.data_file.record_business_rule_violation("LA1", "The language id must be unique.", operation, transaction_id, message_id, record_code, sub_record_code, language_id)
