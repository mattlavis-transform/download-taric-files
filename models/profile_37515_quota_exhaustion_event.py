...
class QuotaExhaustionEvent(object):
    def parse_node(self, app, update_type, message, transaction_id, message_id, record_code, sub_record_code):
        ...
        exhaustion_date = g.app.get_date_value(message, ".//oub:exhaustion.date", True)
        ...
        if g.app.perform_taric_validation is True:
            if quota_definition_sid not in g.app.quota_definitions:
                g.data_file.record_business_rule_violation("QEE1", "The quota definition SID must exist.", operation, transaction_id, message_id, record_code, sub_record_code, str(quota_definition_sid))
